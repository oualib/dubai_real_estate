...
CREATE OR REPLACE FUNCTION FORMAT_DATE AS (x) -> parseDateTimeOrNull(x, '%d-%m-%Y')::Nullable(Date);
...
CREATE OR REPLACE FUNCTION FORMAT_INT AS (x) -> FORMAT_INT_1(FORMAT_INT_0(x))::Nullable(Integer);


CREATE OR REPLACE FUNCTION FORMAT_LICENSE AS (x) ->
    CASE
        WHEN LENGTH(REGEXP_REPLACE(x, '[^0-9]', '')) > 1
        THEN REGEXP_REPLACE(FORMAT_VARCHAR(x), '[^0-9]', '')::Nullable(Integer)
    END;
