...
def _execute_sql(connection: BaseConnection, sql: str, description: str = "", dry_run: bool = False) -> bool:
...
def create_database(
    ...
    with connection:
        return _execute_sql(connection, create_sql, f"Create database '{database_name}'", dry_run)
            ...
            try:
                ...
                for func_name in functions:
                    try:
                        ...
                        if _execute_sql(connection, sql, f"Install {category}.{func_name} function", dry_run):
                        ...
                        logger.error(f"Failed to load function {category}.{func_name}: {e}")
                        stats['errors'] += 1
                        category_stats['errors'] += 1
                ...
                try:
                    ...
                    for sql in category_functions:
                        if _execute_sql(connection, sql, f"Install {category} functions", dry_run):
                            stats['functions_installed'] += 1
                            category_stats['functions'] += 1
                        else:
                            stats['errors'] += 1
    ...
    with connection:
        ...
        for table_name in table_names:
            ...
            try:
                ...
                if _execute_sql(connection, formatted_sql, f"Create table {table_name}", dry_run):
                ...
                if _execute_sql(connection, formatted_sql, f"Ingest data into {table_name}", dry_run):
...
def install_views(
    ...
    with connection:
        for table_name in table_names:
            view_stats = {'table': table_name, 'clean_created': False, 'prod_created': False, 'errors': []}
            ...
            if view_type in ("clean", "both"):
                try:
                    ...
                    if 'clean_view' in [opt.lower() for opt in options]:
                        ...
                        if _execute_sql(connection, formatted_sql, f"Create clean view for {table_name}", dry_run):
            ...
            if view_type in ("prod", "both"):
                try:
                    ...
                    if 'prod_view' in [opt.lower() for opt in options]:
                        ...
                        if _execute_sql(connection, formatted_sql, f"Create prod view for {table_name}", dry_run):
...
def install_database(
    ...
    try:
        ...
        if include_views:
            ...
            view_result = install_views(database_name, "both", table_names, dry_run=dry_run)
